...
export const createAboutMain = async (req, res) => {
  try {
    ...
    if (req.files?.plotImage) {
      const plotRes = await uploadImageToCloudinary(
        req.files.plotImage[0].buffer,
        'aboutus/plot'
      );
    ...
    if (req.files?.acresImage) {
      const acresRes = await uploadImageToCloudinary(
        req.files.acresImage[0].buffer,
        'aboutus/acres'
      );
    ...
    if (req.files?.mainImages) {
      ...
      for (const file of req.files.mainImages) {
        const resCloud = await uploadImageToCloudinary(
          file.buffer,
          'aboutus/mainImages'
        );
...
export const updateAboutMain = async (req, res) => {
  try {
    ...
    if (!aboutMain)
      return res
        .status(404)
        .json({ success: false, message: 'About Main not found' });
    ...
    if (deletedImages.length) {
      aboutMain.mainImages = aboutMain.mainImages.filter(
    ...
    return res.status(200).json({
      success: true,
      message: 'About Main updated successfully',
      aboutMain,
    });
  } catch (err) {
    console.log(err);
...
export const deleteAboutMain = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: 'About Main deleted successfully' });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ success: false, message: err.message });
  }
