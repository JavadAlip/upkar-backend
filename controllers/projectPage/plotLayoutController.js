...
export const updatePlotLayout = async (req, res) => {
  try {
    ...
    if (!layout)
      return res.status(404).json({ message: 'Plot layout not found' });
    ...
    for (let i = 0; i < parsedIcons.length; i++) {
      if (updatedIcons[i]) {
        updatedIcons[i].heading =
          parsedIcons[i].heading || updatedIcons[i].heading;
        updatedIcons[i].subheading =
          parsedIcons[i].subheading || updatedIcons[i].subheading;
      }
    }
