...
export const createFeature = async (req, res) => {
  try {
    ...
    if (!description || !req.files?.mainImage) {
      return res
        .status(400)
        .json({ message: 'Description and main image are required' });
    }
    ...
    if (req.files.icon1 && iconTitle1) {
      const icon1Res = await uploadImageToCloudinary(
        req.files.icon1[0].buffer,
        'features/icons'
      );
      icons.push({ icon: icon1Res.secure_url, iconTitle: iconTitle1 });
    }
    if (req.files.icon2 && iconTitle2) {
      const icon2Res = await uploadImageToCloudinary(
        req.files.icon2[0].buffer,
        'features/icons'
      );
      icons.push({ icon: icon2Res.secure_url, iconTitle: iconTitle2 });
    }
    if (req.files.icon3 && iconTitle3) {
      const icon3Res = await uploadImageToCloudinary(
        req.files.icon3[0].buffer,
        'features/icons'
      );
...
export const updateFeature = async (req, res) => {
  try {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      if (file) {
        const iconUrl = (
          await uploadImageToCloudinary(file.buffer, 'features/icons')
        ).secure_url;
        if (updatedIcons[i - 1]) {
